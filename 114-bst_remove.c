...
/**
 * replace_with_next - Replaces a node with its inorder
 * next in a binary search tree
 * @node: A pointer to the node in the binary search tree
 * Return: A pointer to the node's inorder successor node
 */
bst_t *replace_with_next(bst_t **node)
{
	bst_t *new_node;
 ...
 *
 * Return: A pointer to the new root node, otherwise NULL
 */
bst_t *bst_remove(bst_t *root, int value)
{
	...
	if (new_root != NULL)
	{
		...
		if ((node != NULL) && (node->n == value))
		{
			...
			{
				new_node = replace_with_next(&node);
			}
