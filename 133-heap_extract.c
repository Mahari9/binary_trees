...
/**
 * enqueue_i_4 - Adds an item to a queue
...
/**
 * dequeue_i_4 - Removes an item from a queue.
 * @queue_h: A pointer to the queue's head.
 * @queue_t: A pointer to the queue's tail.
 * @n: A pointer to the queue's size value.
 *
...
/**
 * find_last_heap_node - Gets the last level order node
...
heap_t *find_last_heap_node(const heap_t *root)
{
	...
	heap_t *last_node = NULL, *curr = NULL;
	int n = 0;

	if (root != NULL)
	{
		enqueue_i_4(&head, &tail, &n, (void *)root);
		while (n > 0)
		{
			curr = head;
			if (curr->parent != NULL)
			{
				last_node = curr->parent;
				if (curr->parent->left != NULL)
				{
					enqueue_i_4(&head, &tail, &n, (void *)(curr->parent->left));
				}
				if (curr->parent->right != NULL)
				{
					enqueue_i_4(&head, &tail, &n, (void *)(curr->parent->right));
				}
			}
			dequeue_i_4(&head, &tail, &n);
		}
		while (n > 0)
			dequeue_i_4(&head, &tail, &n);
	}
